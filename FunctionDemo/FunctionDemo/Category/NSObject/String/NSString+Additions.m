...
- (CGSize)sizeWithConstrainedToSize:(CGSize)size fromFont:(UIFont *)font1 lineSpace:(float)lineSpace{
    ...
    //    [self j:string start:0 font:font1];
...
- (void)drawInContext:(CGContextRef)context withPosition:(CGPoint)p andHeight:(float)height andWidth:(float)width{
    NSDictionary *attributesInfo = [self attributesAtIndex:0 effectiveRange:NULL];
    UIFont *font = [attributesInfo valueForKey:NSFontAttributeName];
//    UIColor *color = [attributesInfo valueForKey:NSForegroundColorAttributeName];
    CGSize size = CGSizeMake(width, font.pointSize + 10);
    CGContextSetTextMatrix(context, CGAffineTransformIdentity);
    CGContextTranslateCTM(context, 0,height);
    CGContextScaleCTM(context, 1.0, -1.0);
...
//                                nil];
    ...
    //Create attributed string, with applied syntax highlighting
//    NSMutableAttributedString *attributedStr = [[NSMutableAttributedString alloc] initWithString:self attributes:attributes];
//    [self addAttributes:attributes range:NSMakeRange(0, self.string.length)];
...
- (void)drawInContext:(CGContextRef)context withPosition:(CGPoint)p andSize:(CGSize)size
{
    [self drawInContext:context withPosition:p andHeight:size.height andWidth:size.width];
}
