...
-(WKWebView *)webView
{
    if (_webView == nil) {
        if (self.navigationController.navigationBar.hidden) {
             _webView = [[WKWebView alloc] initWithFrame:CGRectMake( 0, 0, kScreenWidth, kScreenHeight) configuration:self.config];
        }else{
            _webView = [[WKWebView alloc] initWithFrame:CGRectMake( 0, 0, kScreenWidth, kScreenHeight - IPXNAV64 - kBottomBarHeight) configuration:self.config];
        }
...
- (void)loadRequestURL:(NSMutableURLRequest *)request
{
    ...
    /** 插入cookies JS */
    if (Domain) {
        [_webView searchCookieForUserScriptWithDomain:Domain UserScript:^(WKUserScript *script) {
            [self.config.userContentController addUserScript:script];
        }];
    }
    /** 插入cookies PHP */
    if (Domain) {
        [_webView phpCookieStringWithDomain:Domain Callback:^(NSString *domain) {
            [request setValue:domain forHTTPHeaderField:@"Cookie"];
        }];
    }
...
{
    FLog(@">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
    FLog(@" message.body =   %@ ",message.body);
    FLog(@" message.name =   %@ ",message.name);
    FLog(@">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
...
/** 获取本地磁盘的cookies */
- (void)WKSharedHTTPCookieStorage:(void (^)(NSMutableArray *cookieStorage))callback
{
    [self.webView sharedHTTPCookieStorage:^(NSMutableArray *cookie_array) {
        callback(cookie_array);
    }];
}
...
- (void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation
{
    ...
    FLog(@"%@",webView.URL.absoluteString);
...
#pragma clang diagnostic pop
        ...
        [webView evaluateJavaScript:@"window.location.href" completionHandler:^(id _Nullable urlStr, NSError * _Nullable error) {
            if (error == nil) {
                ...
                //FLog(@"currentURLStr : %@ ",_currentURLString);
            }
        }];
        ...
        [webView evaluateJavaScript:heightString4 completionHandler:^(id _Nullable result, NSError * _Nullable error) {
            ...
            FLog(@"html 的高度：%@", result);
        }];
    [self PAWebView:webView didFinishNavigation:navigation];
}
...
- (void)configMenuItem
{
    UIImage *menuImage = CUSTOMIMG(@"notice_icon");
...
#pragma mark - 重新WKWebView构造代理方法
- (void)PAWebView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation
{
