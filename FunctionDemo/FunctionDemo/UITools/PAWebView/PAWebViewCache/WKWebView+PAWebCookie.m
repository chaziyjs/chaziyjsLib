...
{
    [self sharedHTTPCookieStorage:^(NSMutableArray *cookie_array) {
        if (cookie_array.count == 0) return;
        for (NSHTTPCookie *cookie in cookie_array) {
    ...
    @autoreleasepool {
        ...
        if (@available(iOS 12.0, *)) {
            ...
            [cookieStore getAllCookies:^(NSArray *cookies) {
                cookieArray([NSMutableArray arrayWithArray:cookies]);
            }];
        } else {
...
{
    [self jsCookieStringWithDomain:domain Callback:^(NSString *domain) {
        WKUserScript *cookieScript = [[WKUserScript alloc] initWithSource:domain injectionTime:WKUserScriptInjectionTimeAtDocumentStart forMainFrameOnly:NO];
        block(cookieScript);
    }];
}
